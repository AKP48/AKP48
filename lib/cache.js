...
Cache.prototype.addToCache = function(key, data, expires) {
    ...
    this.log.trace({key: key, data: data, expires: expires, expiresEnglish: m.unix(expires).fromNow()}, "Added to cache.");
...
Cache.prototype.getCached = function(key) {
    ...
    if(this.cache[key].expires < (Date.now() / 1000 | 0)) {
        this.log.trace({key: key, data: this.cache[key].data, expires: this.cache[key].expires, expiresEnglish: m.unix(this.cache[key].expires).fromNow()}, "Removed from cache.");
        delete this.cache[key];
        return undefined;
    }

    this.log.trace({key: key, data: this.cache[key].data, expires: this.cache[key].expires, expiresEnglish: m.unix(this.cache[key].expires).fromNow()}, "Fetched from cache.");
