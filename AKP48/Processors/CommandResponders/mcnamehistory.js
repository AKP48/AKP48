...
MCNameHistory.prototype.execute = function(context) {
    ...
    context.apiClient.get("/users/profiles/minecraft/"+context.name, function(err, res, body) {
        if(err || body.error == "Not Found") {context.AKP48.client.say(context.channel, "There is no player with the name "+context.name+"!"); return true;}

        context.apiClient.get("/user/profiles/"+body.id+"/names", function(error, response, bodyy) {
            if(error || body.error == "Not Found" || body.length < 4) {context.AKP48.client.say(context.channel, "There was an error finding previous names for "+context.name+"!"); return true;}
