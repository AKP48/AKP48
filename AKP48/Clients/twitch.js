...
/**
 * The Twitch.TV Client. (honestly just an IRC client with a few changes.)
 * @param {Logger} logger  The logger.
 * @param {Object} AKP48   The running instance of AKP48.
 * @param {Object} client  The raw IRC client to use.
 */
function TwitchClient(logger, AKP48, client) {
    this.log = logger.child({module: "TwitchClient"});
    this.AKP48 = AKP48;
...
TwitchClient.prototype.initialize = function () {
 ...
 */
TwitchClient.prototype.handleRegister = function () {
    var config = this.AKP48.configManager.getServerConfig();
 ...
 */
TwitchClient.prototype.handleAction = function (nick, to, text, message) {
 ...
 */
TwitchClient.prototype.handleKick = function (channel, nick, by, reason) {
 ...
 */
TwitchClient.prototype.handleMessage = function (nick, to, text, message) {
 ...
 * @param  {Object} message The full IRC message object.
 */
TwitchClient.prototype.handleInvite = function (channel, from, message) {
    var self = this;
    //join channel
 ...
 */
TwitchClient.prototype.say = function (channel, msg, directedAt) {
...
/**
 * Send a NOTICE to a user. NOTICEs are not supported on Twitch. Neither are private messages.
 * Honestly not sure what to do about this.
 * @param  {String} user The user to send to.
 * @param  {String} msg  The message to send.
 */
TwitchClient.prototype.notice = function (user, msg) {
    //this.ircClient.notice(user, msg);
    this.log.error({user: user, msg: msg}, "Cannot send NOTICEs on Twitch network!");
};
 ...
 * @param  {String} msg     The message to send.
 */
TwitchClient.prototype.action = function (channel, msg) {
    this.ircClient.action(channel, msg);
};

/**
 * Send a private message to a user. Twitch doesn't support private messages...
 * @param  {String} user The user to send to.
 * @param  {String} msg  The message to send.
 */
TwitchClient.prototype.privmsg = function (user, msg) {
    //this.say(user, msg); //technically the only difference between a private message and a normal one is the recipient.
    this.log.error({user: user, msg: msg}, "Cannot send private messages on Twitch network!");
};
 ...
 */
TwitchClient.prototype.disconnect = function (message) {
 ...
 */
TwitchClient.prototype.join = function (channel, callback) {
 ...
 * @param  {Function} callback Callback to call when finished.
 */
TwitchClient.prototype.part = function (channel, callback) {
 ...
 */
TwitchClient.prototype.getChannels = function () {
 ...
 */
TwitchClient.prototype.changeNick = function (nick) {
 ...
 */
TwitchClient.prototype.getRawClient = function () {
 ...
 */
TwitchClient.prototype.getNick = function () {
    return this.ircClient.nick;
};

module.exports = TwitchClient;
module.exports.clientType = "twitch";
